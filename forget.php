...
-->
                ...
                <form action="" method="POST" id="Regform">
                    ...
                    <input type="text" placeholder="Username" name="username" required>
                    <input type="email" placeholder="E-mail" name="email" required>
                    <button type="submit" class="btn-losi" name="submit">Submit</button>
                ...
                if(isset($_POST['submit'])){
                    ...
                    if(!$db)
                   {
                    ...
                    $count=0;
                   $res= mysqli_query($db,"SELECT * FROM `registration` WHERE Username ='$_POST[username]' && Email='$_POST[email]';");
                   $row= mysqli_fetch_assoc($res);
                   $count=mysqli_num_rows($res);
                   if($count==0)
                   {
                     echo '<script>alert("Username and email is not matched")</script>'; 
                   }else{
                     $_SESSION['email']=$_POST['email'];
                     $_SESSION['otp']=mt_rand(100000,999999);
                     $to=$_POST['email'];
                     $subject="OTP Verification";
                     $msg=" ".$_SESSION['otp']." is your verification code for Recover Password.
                     <br> -Yashogad Society ";
                     if(smtp_mailer($to,$subject,$msg))
                     {
                       
                     }
                     echo '<script>window.location="rec_pass.php"</script>'; 
                   }
                 }
                }
                ?>
